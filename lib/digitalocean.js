 ...
 */
Digitalocean.prototype.get = function(url, parameters, callback, sync) {
	...
	if(sync == true){
		syncReq = httpSync.request({
			method: 'GET',
		    protocol: API_URL.split('://')[0],
		    host: API_URL.replace('https://',''),
		    port: API_URL.split('://')[0] == 'https'?443:80, //443 if protocol = https
		    path: '/' + url + '?' + querystring.stringify(parameters)
		});
		var timedout = false;
		syncReq.setTimeout(10000, function() {
		    console.log("Request Timedout!");
		    timedout = true;
		});
		var response = syncReq.end();

		if (!timedout) {
			body = JSON.parse(response.body.toString());

			if(body.status == 'ERROR'){
				return new Error(body.error_message);
			}
			else{
				return body;
			}
 ...
 * @param  {[boolean]}   privateNetworking Required, a boolean indicating wether we should enable the private networking feature or not.
 * @param  {Function} callback [description]
 */
Digitalocean.prototype.dropletNew = function(name, sizeId, imageId, regionId, sshKeyIds, privateNetworking, callback) {
...
/**
 * Synchronous All Regions
 * This method will return all the available regions within the Digital Ocean cloud.
 * @param  {Function} callback [description]
 */
Digitalocean.prototype.regionGetAllSync = function(callback) {
	res = this.get('regions/', {}, function() {}, true);
...
Digitalocean.prototype.imageGetMineSync = function(callback) {
	res = this.get('images/', {filter: 'my_images'},function(){},true);
	if(res instanceof Error){
		return res;
	}
	else{
...
/**
 * Synchronous version of All SSH Keys
 * This method lists all the available public SSH keys in your account that can be added to a droplet.
 * @param  {Function} callback [description]
 */
Digitalocean.prototype.sshKeyGetAllSync = function(callback) {
	res = this.get('ssh_keys/', {}, function() {}, true);
	if(res instanceof Error){
		return res;
	}
	else{
...
/**
 * All Sizes in a Synchronous way
 * This method returns all the available sizes that can be used to create a droplet synchronously.
 * @param  {Function} callback [description]
 */
Digitalocean.prototype.sizeGetAllSync = function() {
	res = this.get('sizes/', {}, function() {},true);
	if(res instanceof Error){
		return res;
	}
	else{
